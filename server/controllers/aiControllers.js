...
export const generateAGCR = async (req, res) => {
  try {
    const userId = req.userId;
    ...
    if (files.voice && files.voice[0]) {
      try {
        const result = await cloudinary.uploader.upload(files.voice[0].path, {
          resource_type: 'video', // Audio is treated as video in Cloudinary often, or 'auto'
          folder: 'agcr-voice-samples'
        });
        uploadedFiles.voice = result.secure_url;
        // Clean up temp file
    ...
    const masterPrompt = `
  You are Anti-Gravity, an advanced multimodal content generation engine.

  Your task is to generate a complete content package for a 25-second video, including:
  - Video title
  - Description
  - 25-second script (5 scenes, 5 seconds each)
  - Visual descriptions for each scene
  - 2 thumbnail image prompts
  - 4 banner image prompts
  - 6 ad image prompts
  - Ad copy (headlines + primary text)
  - Hashtags
  - Keywords

  Deliver everything in clean JSON format.
  ...
  JSON OUTPUT FORMAT:
  {
    ...
    "image_prompts": {
      "thumbnails": ["Prompt 1", "Prompt 2"],
      "banners": ["Prompt 1", "Prompt 2", "Prompt 3", "Prompt 4"],
      "ads": ["Prompt 1", "Prompt 2", "Prompt 3", "Prompt 4", "Prompt 5", "Prompt 6"]
    },
    "ad_copy": {
      "headlines": ["Headline 1", "Headline 2", "Headline 3"],
      "primary_text": ["Primary Text 1", "Primary Text 2"]
    },
    "hashtags": ["#tag1", "#tag2"],
  ...
  TOPIC: ${topic}
  VOICE_SAMPLE_URL: ${uploadedFiles.voice || 'None'}
  REFERENCE_IMAGE_URLS: ${JSON.stringify(uploadedFiles.images)}
  `;
    ...
    // Construct specific payload for n8n
    const webhookPayload = {
      voice_url: uploadedFiles.voice,
      image_urls: uploadedFiles.images,
      video_title: parsedContent.title,
      video_script: parsedContent.video_script,
    ...
    try {
      ...
      await axios.post('https://speedcubers345.app.n8n.cloud/webhook-test/3877f5f8-b0fe-410b-beb3-49159940e565', webhookPayload);
    ...
    await query(
      `INSERT INTO creations (user_id, prompt, content, type)
        VALUES ($1, $2, $3, 'agcr-engine')`,
      [userId, topic, JSON.stringify(parsedContent)]
    );
    ...
    console.error("Error in generateAGCR:", error);
    res.status(500).json({ success: false, message: error.message, stack: error.stack });
  }
};

export const getAGCRHistory = async (req, res) => {
  try {
    const userId = req.userId;
    const result = await query(
      `SELECT * FROM creations WHERE user_id = $1 AND type = 'agcr-engine' ORDER BY created_at DESC`,
      [userId]
    );

    res.json({ success: true, history: result.rows });
  } catch (error) {
    console.error("Error fetching AGCR history:", error);
...
export const generateBanner = async (req, res) => {
  try {
    ...
    if (!topic || !platform) {
      return res.status(400).json({
        success: false,
        message: "Please provide a topic and platform"
      });
    }

    // Construct optimized prompt based on platform
    // Construct optimized prompt based on platform
    let optimizedPrompt = "";

    // Platform-specific prompt engineering
    const prompts = {
      'LinkedIn Post': `Professional LinkedIn post image for ${topic}, ${style} style, square or portrait aspect ratio, corporate, clean, high resolution, 8k, no text`,
      'LinkedIn Banner': `Professional LinkedIn banner background for ${topic}, ${style} style, wide aspect ratio (4:1), minimalist, corporate, high resolution, 8k, no text`,
      'Instagram Post': `Instagram post image for ${topic}, ${style} style, square aspect ratio (1:1), aesthetic, trendy, high quality, 8k, no text`,
      'Twitter Post': `Twitter post image for ${topic}, ${style} style, landscape aspect ratio (16:9), engaging, high quality, 8k, no text`,
      'Twitter Banner': `Twitter header image for ${topic}, ${style} style, wide aspect ratio (3:1), professional, high quality, 8k, no text`
    };

    optimizedPrompt = prompts[platform] || `${platform} image for ${topic}, ${style} style, high quality, no text`;

    console.log('Generating banner with prompt:', optimizedPrompt);

    // Reuse the Clipdrop API logic (similar to generateImage)
    const formData = new FormData();
    ...
    await query(
      `INSERT INTO creations (user_id, prompt, content, type)
       VALUES ($1, $2, $3, 'banner')`,
      [userId, `${platform} Banner: ${topic}`, uploadResult.secure_url]
    );
    ...
    res.status(500).json({
      success: false,
      message: "Failed to generate banner. Please try again.",
      error: error.message
    });
  }
};
...
export const chatWithGemini = async (req, res) => {
  try {
    ...
    // Filter out any messages that don't have role 'user' or 'assistant'
    const validHistory = history ? history.filter(msg => ['user', 'assistant'].includes(msg.role)) : [];

    const response = await AI.chat.completions.create({
      model: "gemini-1.5-flash", // Use 1.5-flash as it is stable
