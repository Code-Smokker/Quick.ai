      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-slate-800 p-4 rounded-lg border border-slate-700'>
          ...
          {imageStyles.map((item) => (
            <span
              ...
              className={`text-sm px-4 py-1 border rounded-full cursor-pointer transition-colors
                ${selectedStyle === item ? 'bg-green-500/20 text-green-400 border-green-500/50' : 'text-slate-400 border-slate-600 hover:bg-slate-700'}
        ...
        <div className='my-6 items-center flex gap-2'>
          <label className='relative cursor-pointer'>
            ...
            />
            <div className='w-9 h-5 bg-slate-700 rounded-full peer-checked:bg-green-500 transition'></div>
            <span className='absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition peer-checked:translate-x-4'></span>
          </label>
          <p className='text-sm text-slate-300'>Make this image Publish</p>
        </div>
        ...
        {!imageUrl ? (
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-slate-500'>
              <Image className='w-9 h-9 opacity-50' />
              <p>Enter a topic and click "Generate Image" to get started</p>
            </div>
          </div>
        ) : (
          <div className='mt-3 h-full'>
            <img src={imageUrl} alt='image' className='w-full h-full rounded-md shadow-lg' />
          </div>
        )}
      </div>
    </div>
